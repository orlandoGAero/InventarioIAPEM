...
<html xmlns="http://www.w3.org/1999/xhtml" dir="ltr" lang="es">
	<head>
		<title>Registrar Inventario Bienes Muebles</title>
	...
	<body>
		<div id="contenedor">
			...
			<div id="contenido">
				...
				<div id="derecha3">
					<div id="botones_sec">
						<a href="registrar_muebles.php"><img src='../../imagenes/mueble.png' ALT="Nuevo Artículo" TITLE="Nuevo Artículo" vspace=4 hspace=4/></a>
						<a href="registrar_inventario-muebles.php"><img src='../../imagenes/inventario.png' ALT="Inventario" TITLE="Registrar Inventario" vspace=4 hspace=4/></a>
					</div>
					
					<center>
						<fieldset><legend>Registro de Inventario Bienes Muebles</legend>
							<form name='altaIM' action='' method='POST' id='altaIM' target='_self'>
								<ul>
									<li>
									...
									<li>
										<?php
											include '../../librerias/conexion.php';
											$consultaI = "SELECT IDinmueble,nombre_sede FROM bienes_inmuebles WHERE activo='Si' ORDER BY nombre_sede";
											$ejecutarI = mysql_query($consultaI) or die (mysql_error());
											$filasI = mysql_num_rows($ejecutarI);
											echo"<select name='idInm' class='select'><option selected>Seleccione...</option>";
												for($y=0;$y<$filasI;$y++)
												{
													$idInm = mysql_result($ejecutarI,$y,'IDinmueble');
													$nomInm = mysql_result($ejecutarI,$y,'nombre_sede');
													$nomInm = utf8_encode($nomInm);
													echo"<option value='$idInm'>$nomInm</option>";
												}
										?>
												</select>
									</li>
									...
									<li>	
										<?php
											$consultaC = "SELECT IDcoordinacion,coordinacion FROM coordinaciones WHERE activo='Si' ORDER BY coordinacion";
									...
									<li>
										<table border='0' align='left'>
											<tr>
												<td>
													<input type='text' name='nuevoCodM' size='30' class = 'rounded' onKeyUp='codnuevo(this.value,co);'>
												</td>
									...
									<li>
										<table border='0' align='left'>
											<tr>
												<td>
													<input type='text' name='anteriorCodM' size='30' class = 'rounded' onKeyUp='codanterior(this.value,coda);'>
												</td>
									...
									<li>
										<?php
											$consultaM = "SELECT IDmueble,nombre FROM muebles WHERE activo='Si' ORDER BY nombre";
											$ejecutarM = mysql_query($consultaM) or die (mysql_error());
											$filasM = mysql_num_rows($ejecutarM);
											echo"<select name='idM' class = 'select'><option selected>Seleccione...</option>";
												for($y=0;$y<$filasM;$y++)
												{
									...
									<li>
										<table border='0' align='left'>
											<tr>
												<td>
													<input type='text' name='area' size='30' class = 'rounded' onKeyUp='letras(this.value,ar);'>
												</td>
					...
					<script type="text/javascript">
						$(function (e) {
							$('#altaIM').submit(function (e) {
								e.preventDefault()
								$('#ajax').load('registrar_inventario-muebles2.php?' + $('#altaIM').serialize())
							})
						})
					</script>
				</div>
