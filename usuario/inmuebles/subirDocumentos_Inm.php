		...
		<div id="contenedor">
			...
			<div id="contenido">
				...
				<div id="derecha6">
					...
					<center>
						<fieldset><legend>Subir Documento</legend>
							<!--paso 1 : colocar  EL ENCTYPE ENB EL FORMULARIO-->
							<!--paso 2 : SIEMPRE SE OCUPA EL POST-->
							<!--paso 3 : EN ESTE FORMULARIO NO SE PUEDE HACER AJAX-->
							<form action='subirDocumentos_Inm2.php?idInm' method='POST' enctype='multipart/form-data'>
								<ul>
									<li>
										<?php
											include '../../librerias/conexion.php';
											$consulta = "SELECT IDdoc_inmuebles FROM documentos_inmuebles ORDER BY IDdoc_inmuebles DESC LIMIT 1";
											$ejecutar = mysql_query($consulta) or die (mysql_error());
											$filas = mysql_num_rows($ejecutar);
												...
												{
													$idDocInm = mysql_result($ejecutar,0,'IDdoc_inmuebles');
													$idDocInm = $idDocInm+1;
												}
											echo"<input type='hidden' name='iddoc' value='$idDocInm'>";
										?>
									</li>
									<li>
									...
									<li>
										<select name='idInm' id='idIn' class='select'><option selected>Seleccione...</option>
											<?php
												$consultaInm = "SELECT bi.IDinmueble,bi.nombre_sede,ea.tipo
												...
												$filasInm = mysql_num_rows($ejecutarInm);
													for($y=0;$y<$filasInm;$y++)
													{
														$idInm = mysql_result($ejecutarInm,$y,'IDinmueble');
														$nomInm = mysql_result($ejecutarInm,$y,'nombre_sede');
									...
									<li>
										<!--paso 4 : INPUT TYPE FILE , PONER NOMBRE Y PONER ID-->
										<input type='file'  name='archivo' id='archivo' class='rounded'>
									</li>
