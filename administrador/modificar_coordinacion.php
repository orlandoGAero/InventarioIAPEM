...
<html xmlns="http://www.w3.org/1999/xhtml" dir="ltr" lang="es">
	<head>
		<title>Modificar Coordinación</title>
	...
	<body>
		<div id="contenedor">
			...
			<div id="contenido">
				...
				<div id="derecha7">
					<div id="botones_sec">
						<a href="buscar_coordinacion.php"><img src='../imagenes/back.png' ALT="Regresar" TITLE="Regresar" vspace=4 hspace=4/></a>
					</div>
					...
					<?php
						...
						$consultaUs = "SELECT u.nombre,u.appat,u.ammat,c.fecha_mod
									   FROM coordinaciones AS c,usuarios AS u
									   WHERE c.IDcoordinacion=$idCo AND c.IDusuario=u.IDusuario";
						...
						/*--*/
						$consulta2 = "SELECT IDcoordinacion,coordinacion,activo FROM coordinaciones WHERE IDcoordinacion = '$idCo'";
						$ejecutar2 = mysql_query($consulta2) or die (mysql_error());
						
						$idCo = mysql_result($ejecutar2,0,'IDcoordinacion');
						$nomCo = mysql_result($ejecutar2,0,'coordinacion');
						$nomCo = utf8_encode($nomCo);
						$activo = mysql_result($ejecutar2,0,'activo');
					?>				
					
					<center>
						<fieldset><legend>Modificación de Coordinación</legend>
							<form name='modCo' action='' method='POST' id='modCo' target='_self'>
								<ul>
									<li>
										<label>Clave de Coordinación:</label>
									</li>
									<li>
										<?php
											echo"<input type='text' name='idCo' value='$idCo' readonly='readonly' size='30' class = 'rounded'>";
										?>
									</li>
									...
									<li>
										<table border='0' align='left'>
											<tr>
												<td>
													<?php
														echo"<input type='text' name='nomCo' value='$nomCo' size='30' class='rounded' onKeyUp='letras(this.value,nomC)'>";
													?>
												</td>
					...
					<script type="text/javascript">
						$(function (e) {
							$('#modCo').submit(function (e) {
								e.preventDefault()
								$('#ajax').load('modificar_coordinacion2.php?' + $('#modCo').serialize())
							})
						})
					</script>
				</div>
