			...
			<div id="contenido">
				...
				<div id="derecha8">
					...
					<?php	
						...
						$consulta = "SELECT bi.IDinmueble,bi.nombre_sede,bi.calle,bi.numero,bi.colonia,bi.cp,bi.ciudad,bi.telefono,bi.fecha_ingreso,r.nombre,r.ap_paterno,r.ap_materno,ea.tipo
									FROM bienes_inmuebles AS bi,resguardatarios AS r,estado_adquisicion AS ea
									WHERE bi.IDinmueble=$idInm AND r.IDresguardatario=bi.IDresguardatario AND ea.IDest_adq=bi.IDest_adq";
						$ejecutar = mysql_query($consulta) or die (mysql_error());
						
						$idInm = mysql_result($ejecutar,0,'IDinmueble');
						$nombre = mysql_result($ejecutar,0,'nombre_sede');
						$nombre = utf8_encode($nombre);
						$calle = mysql_result($ejecutar,0,'calle');
						$calle = utf8_encode($calle);
						$numero = mysql_result($ejecutar,0,'numero');
						$colonia = mysql_result($ejecutar,0,'colonia');
						$colonia = utf8_encode($colonia);
						$cp = mysql_result($ejecutar,0,'cp');
						$ciudad = mysql_result($ejecutar,0,'ciudad');
						...
						$telefono = mysql_result($ejecutar,0,'telefono');
						$nomR = mysql_result($ejecutar,0,'nombre');
						$nomR = utf8_encode($nomR);
						$apR = mysql_result($ejecutar,0,'ap_paterno');
						$apR = utf8_encode($apR);
						$amR = mysql_result($ejecutar,0,'ap_materno');
						$amR = utf8_encode($amR);
						$nombreR = "$nomR $apR $amR";
						$estado = mysql_result($ejecutar,0,'tipo');
						...
						$consultaE = "SELECT IDest_adq,tipo FROM estado_adquisicion WHERE tipo != '$estado' ORDER BY tipo";
						$ejecutarE = mysql_query($consultaE) or die (mysql_error());
						$filasE = mysql_num_rows($ejecutarE);
						
						echo"<div id='ajaxCambiarEA'>";
							echo "<center>
									<fieldset><legend>Cambiar Estado de Adquisición</legend>
										<form action='' method='POST' id='cambiarEA' target='_self'>
											<ul>
												<li>
						...
						echo"</div>
							...
							<script type='text/javascript'>
								$(function (e) {
									$('#cambiarEA').submit(function (e) {
										e.preventDefault()
										$('#ajaxCambiarEA').load('cambiar_estado-adq.php?' + $('#cambiarEA').serialize())
									})
								})
							</script>";
					?>
