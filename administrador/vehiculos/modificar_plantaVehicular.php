<!DOCTYPE HTML PUBLIC "- / / W3C / / DTD HTML 4.01 Transitional / / EN""Http://www.w3.org/TR/html4/loose.dtd"> 
<html xmlns="http://www.w3.org/1999/xhtml" dir="ltr" lang="es">
	<head>
		<title>Modificar Planta Vehícular</title>
		<link rel = "shortcut icon" type = "image/x-icon" href = "../../imagenes/inventarioiapem2.png" />
		<meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
		<link rel="stylesheet" href="../../css/style_divs.css" type="text/css"/>
		<link rel="stylesheet" href="../../css/stylemenu.css" type="text/css">
		<link rel="stylesheet" href="../../css/style_menu_vert.css" type="text/css" /><style type="text/css">._css3m{display:none}</style>
		<link rel="stylesheet" href="../../css/estiloform.css" type="text/css">
		<link rel="stylesheet" href="../../css/ayuda.css" type="text/css">
		<link type="text/css" href="../../css/style_table.css" rel="stylesheet"/><!--Estilo tabla-->
		<!--<link rel="stylesheet" type="text/css" href="../../css/ayuda.css"/>-->
		<script type="text/javascript" src="../../librerias/jquery-1.4.2.min.js"></script>
		<script type="text/javascript" src="../../librerias/validacion2.js"></script>
		<?php @session_start();
			if ($_SESSION['acceso'] == "" or $_SESSION['IDusuario'] == "")
			{
				print  "<meta http-equiv = Refresh content = \"0; url = ../../index.php\">";
			}
			$acceso = $_COOKIE ['acceso'];
			$idu = $_COOKIE ['IDusuario'];
			$tipo = $_COOKIE ['tipo'];
			if($acceso != '1')
			{
				print  "<meta http-equiv = Refresh content = \"0; url = ../../index.php\">";
			}
			include '../../librerias/conexion.php';
			$consulta = " 	SELECT *
							FROM usuarios
							WHERE IDusuario = '$idu' ";
			$ejecutar = mysql_query ($consulta) or die (mysql_error());
				$usuario = mysql_result ($ejecutar, 0, 'usuario');
				$nombre = mysql_result ($ejecutar, 0, 'nombre');
				$nombre = utf8_encode($nombre);
				$ap = mysql_result ($ejecutar, 0, 'appat');
				$ap = utf8_encode($ap);
				$am = mysql_result ($ejecutar, 0, 'ammat');
				$am = utf8_encode($am);
				$nombreUs = "$nombre $ap $am";
		?>
			<script>
			!window.jQuery && document.write('<script src="../../librerias/jquery-1.4.3.min.js"><\/script>');
		</script>
		<script type = "text/javascript" src = "../../librerias/jquery.fancybox-1.3.4.pack.js"></script>
		<link rel="stylesheet" type="text/css" href="../../css/jquery.fancybox-1.3.4.css" media = "screen" />	
			<script type = "text/javascript">
			$(document).ready(function() 
			{
				$("#iapem").fancybox();
			});
			</script>
		<script language = "JavaScript">
			<!-- Begin
			function popUp(URL) 
			{
				day = new Date();
				id = day.getTime();
				eval("page" + id + " = window.open(URL, '" + id + "', 'toolbar=1 ,scrollbars=1,location=1,statusbar=1,menubar=1,resizable=0,width=100,height=300,left = 0,top = 0');");
			}
			// End -->
		</script>
		<!--Calendario-->
		<script type="text/javascript" src="../../calendario/js/jquery-ui-1.8.2.custom.min.js"></script>
		<link href="../../calendario/css/ui-darkness/jquery-ui-1.8.2.custom.css" rel="stylesheet" type="text/css" />
		<script type="text/javascript">
			$(function() {
				$(".date").datepicker(
					{
						dateFormat:'yy-mm-dd'
						, monthNames: ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre']
						, dayNamesMin: ['Do', 'Lu', 'Ma', 'Mi', 'Ju', 'Vi', 'Sá']
					}
				);
			});
		</script> 
	</head>
	<body>
		<div id="contenedor">
			<div id="banner">
				<img src="../../imagenes/encab-iapem1.png" width='100%' class = 'bordeimagen'/>
			</div>
		
			<div id="cssmenu">
				<ul class="menu">
					<li  class='active'><a href="../indexadmin.php">Inicio</a></li>
					
					<li class='has-sub'><a href="#">Coordinación</a>
						<ul>
							<li><a href="../registro_coordinacion.php">Registrar</a></li>
							<li><a href="../buscar_coordinacion.php">Buscar</a></li>
						</ul>
					</li>
					
					<li class='has-sub'><a href="#">Responsable</a>
						<ul>
							<li><a href="../registro_responsable.php">Registrar</a></li>
							<li><a href="../buscar_responsable.php">Buscar</a></li>
						</ul>
					</li>
					
					<li class='has-sub'><a href="#">Usuarios</a>
						<ul>
							<li><a href="../registrar_usuario.php">Registrar</a></li>
							<li><a href="../buscar_usuario.php">Buscar</a></li>
						</ul>
					</li>
					
					<li class='has-sub'><a href="#">Reportes</a>
						<ul>
							<li><a href="../reportespdf/reporte_responsables.php">Responsable</a></li>
							<li><a href="../reportespdf/reporte_coordinaciones.php">Coordinación</a></li>
						</ul>
					</li>

					<li>
						<div style="display: none;">
							<div id = "about" style = "width:500px;height:300;overflow:auto;background:transparent;">
								<img src = '../../imagenes/Acerca.png' height = '300' width = '500'>
							</div>
						</div>
						<a id = 'iapem' href = '#about'>Acerca de</a>
					</li>
					
					<li class = 'has-sub'><a href="#"> <?php echo "$usuario" ?> </a>
						<ul>
							<li><a href="../cambiar_contrasena.php">Cambiar Contraseña</a></li>
							<li><a href="../../logout.php">Salir</a></li>
						<ul>
					</li>
				</ul>
			</div>
			
			<div id="contenido">

				<div id="izquierda2">
					<ul id="css3menu1" class="topmenu">
						<li class="topfirst"><a href="#" style="width:139px;"><span>Bienes Muebles</span></a>
							<ul>
								<li class="subfirst"><a href="../muebles/registrar_muebles.php"><img src="data:image/png;base64," alt=""/>Registrar</a></li>
								<li><a href="../muebles/buscar_muebles.php"><img src="data:image/png;base64," alt=""/>Buscar</a></li>
							</ul>
						</li>
						
						<li class="topmenu"><a href="#" style="width:139px;"><span>Bienes Inmuebles</span></a>
							<ul>
								<li class="subfirst"><a href="../inmuebles/registrar_Inmuebles.php"><img src="data:image/png;base64," alt=""/>Registrar</a></li>
								<li><a href="../inmuebles/buscar_Inmuebles.php"><img src="data:image/png;base64," alt=""/>Buscar</a></li>
							</ul>
						</li>
						
						<li class="topmenu"><a href="#" style="width:139px;"><span>Bienes Informáticos</span></a>
							<ul>
								<li class="subfirst"><a href="../tecnologias/registrar_tecnologias.php"><img src="data:image/png;base64," alt=""/>Registrar</a></li>
								<li><a href="../tecnologias/buscar_artInformaticos.php"><img src="data:image/png;base64," alt=""/>Buscar</a></li>
							</ul>
						</li>
						
						<li class="topmenu"><a href="#" style="width:139px;"><span>Planta Vehicular</span></a>
							<ul>
								<li class="subfirst"><a href="registrar_plantaVehicular.php"><img src="data:image/png;base64," alt=""/>Registrar</a></li>
								<li><a href="buscar_vehiculo.php"><img src="data:image/png;base64," alt=""/>Buscar</a></li>
							</ul>
						</li>
					</ul>
				</div>
				
				<!-- Fin de menú -->

					<div id="derecha2">
						<div id="botones_sec">
							<a href="buscar_vehiculo.php"><img src='../../imagenes/back.png' ALT="Regresar" TITLE="Regresar" vspace=4 hspace=4/></a>
						</div>

										<?php

											include '../../librerias/conexion.php';
											include '../../librerias/decode.php';
						
												decode_get($_SERVER["REQUEST_URI"]);
										
											include '../../librerias/quitar.php';

											//print_r($_GET);
											$codigoNuevoV = mysql_real_escape_string (quitar($_GET ['codigoNuevoV']));
											
												/*consulta para obtener el nombre de usuario que realizo la última modificación*/
													$consultaUs = "SELECT u.nombre,u.appat,u.ammat,pv.fecha_mod
																   FROM planta_vehicular AS pv,usuarios AS u
																   WHERE pv.codigoNuevoV='$codigoNuevoV' AND u.IDusuario=pv.IDusuario";
													$ejecutarUs = mysql_query($consultaUs) or die (mysql_error());
													$nomUs = mysql_result($ejecutarUs,0,'nombre');
													$nomUs = utf8_encode($nomUs);
													$apUs = mysql_result($ejecutarUs,0,'appat');
													$apUs = utf8_encode($apUs);
													$amUs = mysql_result($ejecutarUs,0,'ammat');
													$amUs = utf8_encode($amUs);
													$nombreUs = "$nomUs $apUs $amUs";
													$fechaMod = mysql_result($ejecutarUs,0,'fecha_mod');
													
													echo"<table id='miTabla'>
															<tr>
																<th colspan='2'>Registro de modificación</th>
															</tr>
															<tr>
																<th>Fecha de última modificación:</th><td>$fechaMod</td>
															</tr>
															<tr>
																<th>Realizada por:</th><td>$nombreUs</td>
															</tr>
														</table><br/>";
														
												/*--*/

											$consulta = " SELECT pve.*, coor.IDcoordinacion, coor.coordinacion,	resg.IDresguardatario, resg.nombre, resg.ap_paterno, resg.ap_materno, inmueble.IDinmueble, inmueble.nombre_sede
														  FROM planta_vehicular as pve, coordinaciones AS coor, resguardatarios AS resg, bienes_inmuebles AS inmueble
														 WHERE codigoNuevoV = '$codigoNuevoV'
														 AND coor.IDcoordinacion = pve.IDcoordinacion
														 AND resg.IDresguardatario = pve.IDresguardatario
														 AND inmueble.IDinmueble = pve.IDinmueble";
											$ejecutar = mysql_query($consulta) or die (mysql_error());

											$IDinmueble = mysql_result($ejecutar, 0, 'IDinmueble');
											$Inmueble = mysql_result($ejecutar, 0, 'nombre_sede');
												$Inmueble = utf8_encode($Inmueble);
											$IDcoord = mysql_result($ejecutar, 0, 'IDcoordinacion');
											$Coord = mysql_result($ejecutar, 0, 'coordinacion');
												$Coord = utf8_encode($Coord);
											$IDresg = mysql_result($ejecutar, 0, 'IDresguardatario');
											$nombre = mysql_result($ejecutar, 0, 'nombre');
												$nombre = utf8_encode($nombre);
											$ap = mysql_result($ejecutar, 0, 'ap_paterno');
												$ap = utf8_encode($ap);
											$am = mysql_result($ejecutar, 0, 'ap_materno');
												$am = utf8_encode($am);
											$Resguard = " $nombre $ap $am ";
											$fecha_in = mysql_result($ejecutar, 0, 'fecha_ingreso');
											$codigoNuevoV = mysql_result($ejecutar, 0, 'codigoNuevoV');
											$codigoAnterior = mysql_result($ejecutar, 0, 'codigoAnterior');
											$modelo = mysql_result($ejecutar, 0, 'modelo');
												$modelo = utf8_encode($modelo);
											$marca = mysql_result($ejecutar, 0, 'marca');
												$marca = utf8_encode($marca);
											$color = mysql_result($ejecutar, 0, 'color');
												$color = utf8_encode($color);
											$placas = mysql_result($ejecutar, 0, 'placas');
												$placas = utf8_encode($placas);
											$noSerie = mysql_result($ejecutar, 0, 'noSerie');
											$ano = mysql_result($ejecutar, 0, 'ano');
											$ultVer = mysql_result($ejecutar, 0, 'fechaUltima_verificacion');
											$proVer = mysql_result($ejecutar, 0, 'fechaProxima_verificacion');
											$ultTen = mysql_result($ejecutar, 0, 'fechaUltima_tenencia');
											$proTen = mysql_result($ejecutar, 0, 'fechaProxima_tenencia');
											$TarjetaCirculacion = mysql_result($ejecutar, 0, 'noTarjetaCirculacion');
											$tipo_v = mysql_result($ejecutar, 0, 'tipo_v');
												$tipo_v = utf8_encode($tipo_v);
											$servicio = mysql_result($ejecutar, 0, 'servicio');
											$activo = mysql_result($ejecutar, 0, 'activo');

										?>

							<center>
								<fieldset><legend>Modificación de Planta Vehícular</legend>
									<form action = '' name = 'modPv' method = 'POST' id = 'modPv' target = '_self'>
										<ul>

											<li>
									        	<label>Inmueble:</label>
									        </li>
									        <li>
									        	<?php

									        			echo "<select name = 'IDinmueble' class = 'select'>
									        					<option value = '$IDinmueble'>$Inmueble</option>";
												
													$consultaI = "	SELECT IDinmueble,nombre_sede 
																	FROM bienes_inmuebles
																	WHERE IDinmueble != $IDinmueble";
									        		$ejecutarI = mysql_query($consultaI) or die (mysql_error());
									        		$filasI = mysql_num_rows($ejecutarI);
									        		
									        			for($y=0;$y<$filasI;$y++)
									        				{
									        					$idinmueble = mysql_result($ejecutarI,$y,'IDinmueble');
									        					$nombreS = mysql_result($ejecutarI,$y,'nombre_sede');
									        					$nombreS = utf8_encode($nombreS);
									        					echo "<option value = '$idinmueble'>$nombreS</option>";
									        				}
									        	?>
									        			</select>
									        </li>											

											<li>
												<label>Coordinación o Área:</label>			
											</li>
											<li>	
												<?php

														echo " <select name = 'IDcoordinacion' class = 'select'>
																<option value = '$IDcoord'>$Coord</option> ";
													
													$consultaC = "	SELECT IDcoordinacion,coordinacion 
																	FROM coordinaciones
																	WHERE IDcoordinacion != $IDcoord";
													$ejecutarC = mysql_query($consultaC) or die (mysql_error());
													$filasC = mysql_num_rows($ejecutarC);
													
														for($y=0;$y<$filasC;$y++)
														{
															$idCo = mysql_result($ejecutarC,$y,'IDcoordinacion');
															$nomCo = mysql_result($ejecutarC,$y,'coordinacion');
															$nomCo=utf8_encode($nomCo);
															echo"<option value = '$idCo'>$nomCo</option>";
														}
													?>
														</select>	
											</li>
												
											<li>
												<label>Responsable:</label>		
											</li>
											<li>
												<?php

													echo " <select name = 'IDresguardatario' class = 'select'>
																<option value = '$IDresg'>$Resguard</option>";

													$consultaR = "	SELECT *  
																	FROM resguardatarios
																	WHERE IDresguardatario != $IDresg";
													$ejecutarR = mysql_query($consultaR) or die (mysql_error());
													$filasR = mysql_num_rows($ejecutarR);
													
														for($y=0;$y<$filasR;$y++)
														{
															$idR = mysql_result($ejecutarR,$y,'IDresguardatario');
															$nombre = mysql_result($ejecutarR,$y,'nombre');
															$nombre = utf8_encode($nombre);
															$ap = mysql_result($ejecutarR,$y,'ap_paterno');
																$ap = utf8_encode($ap);
															$am = mysql_result($ejecutarR,$y,'ap_materno');
																$am = utf8_encode($am);
															$nombreRe = "$nombre $ap $am";
															echo"<option value = '$idR'>$nombreRe</option>";
														}
													?>
														</select>
											</li>
												
											<li>
												<label>Fecha Ingreso: <font color = '#FF0000'>*</font></label>
											</li>
											<li>
												<input type = 'text' name = 'fecha' <?php echo "value = '$fecha_in' "; ?> size = '30' class = 'ancho date rounded calend'>		
											</li>
														
											<li>
												<label>Código Nuevo: <font color = '#FF0000'>*</font></label>
											</li>
											<li>
												<table border = '0' align='left'>
													<tr>
														<td>
															<input type = 'text' name = 'nuevoCod' <?php echo "value = '$codigoNuevoV' "; ?> size = '30' class = 'rounded' OnKeyUp = 'codnuevo(this.value,codin)'>
														</td>
														<td id = 'codin' width = '22'>  </td>
														<td> <a class = "tooltip" > <font size = '4' color = '#0000FF'>(?)</font> <span class="custom help"> <img src = "../../imagenes/Help.png" alt = "Help" height="35" width="35" />Ingresa el Código Nuevo del vehículo, por ejemplo:<br>TOLHP111001-00</span></a></td>
													</tr>
												</table>
											</li>
												
											<li>
												<label>Código Anterior:</label>
											</li>
											<li>
												<table border = '0' align = 'left'>
													<tr>
														<td>
															<input type = 'text' name = 'anteriorCod' <?php echo "value = '$codigoAnterior' "; ?> size = '30' class = 'rounded' OnKeyUp = 'codanterior(this.value,codante)'>
														</td>
														<td id = 'codante' width = '22'>  </td>
														<td> <a class = "tooltip" > <font size = '4' color = '#0000FF'>(?)</font> <span class="custom help"> <img src = "../../imagenes/Help.png" alt = "Help" height="35" width="35" />Ingresa el Código Anterior del vehículo, por ejemplo:<br>MORP11009-00</span></a></td>
													</tr>
												</table>
											</li>
											
											<li>
												<label>Modelo: <font color = '#FF0000'>*</font></label>
											</li>
											<li>
												<table border = '0' align = 'left'>
													<tr>
														<td>
															<input type = 'text' name = 'modelo' <?php echo "value = '$modelo' "; ?> size = '30' class = 'rounded'>
														</td>
														<td>  </td>
													</tr>
												</table>
											</li>
											
											<li>
												<label>Marca: <font color = '#FF0000'>*</font></label>
											</li>
											<li>
												<table border = '0' align = 'left'>
													<tr>
														<td>
															<input type = 'text' name = 'marca' <?php echo "value = '$marca' "; ?> size = '30' class = 'rounded' OnKeyUp = 'automarca(this.value,marc)'>
														</td>
														<td id = 'marc' width = '22'>  </td>
													</tr>
												</table>
											</li>
											
											<li>
												<label>Color: <font color = '#FF0000'>*</font></label>
											</li>
											<li>
												<table border = '0' align = 'left'>
													<tr>
														<td>
															<input type = 'text' name = 'colorVe' <?php echo "value = '$color' "; ?> size = '30' class = 'rounded' onKeyUp = 'color(this.value,colorV)'>
														</td>
														<td id = 'colorV' width = '22'>  </td>
													</tr>
												</table>
											</li>
											
											<li>
												<label>Placas: <font color = '#FF0000'>*</font></label>
											</li>
											<li>
												<table border = '0' align = 'left'>
													<tr>
														<td>
															<input type = 'text' name = 'placas' <?php echo "value = '$placas' "; ?> size = '30' class = 'rounded' OnKeyUp = 'placasV(this.value,placs)'>
														</td>
														<td id = 'placs' width = '22'>  </td>
														<td> <a class = "tooltip" > <font size = '4' color = '#0000FF'>(?)</font> <span class="custom help"> <img src = "../../imagenes/Help.png" alt = "Help" height="35" width="35" />Ingresa las placas del vehículo, por ejemplo:<br>123-ABC<br>ABC-12-34</span></a></td>
													</tr>
												</table>
											</li>
											
											<li>
												<label>NoSerie<font color = '#FF0000'>*</font></label>
											</li>
											<li>
												<table border = '0' align = 'left'>
													<tr>
														<td>
															<input type = 'text' name = 'noserie' <?php echo "value = '$noSerie' "; ?> size = '30' class = 'rounded' OnKeyUp = 'vin(this.value,nser)'>
														</td>
														<td id = 'nser' width = '22'>  </td>
													</tr>
												</table>
											</li>
											
											<li>
												<label>Año: <font color = '#FF0000'>*</font></label>
											</li>
											<li>
												<table border = '0' align = 'left'>
													<tr>
														<td>
															<input type = 'text' name = 'ano' <?php echo "value = '$ano' "; ?> size = '30' class = 'rounded' maxlength = '4' OnKeyUp = 'year(this.value,ye)'>
														</td>
														<td id = 'ye' width = '22'>  </td>
													</tr>
												</table>
											</li>
											
											<li>
												<label>Ultima Verificación: <font color = '#FF0000'>*</font></label>
											</li>
											<li>
												<input type = 'text' name = 'fecha_ultima_v' <?php echo "value = '$ultVer' "; ?> size = '30' class = 'ancho date rounded calend'>		
											</li>
											
											<li>
												<label>Proxima Verificación: <font color='#FF0000'>*</font></label>
											</li>
											<li>
												<input type = 'text' name = 'fecha_proxima_v' <?php echo "value = '$proVer' "; ?> size = '30' class = 'ancho date rounded calend'>		
											</li>
											
											<li>
												<label>Fecha Ultima Tenencia: <font color = '#FF0000'>*</font></label>
											</li>
											<li>
												<input type = 'text' name = 'fecha_ultima_t' <?php echo "value = '$ultTen' "; ?> size = '30' class = 'ancho date rounded calend'>		
											</li>
											
											<li>
												<label>Fecha Proxima Tenencia: <font color = '#FF0000'>*</font></label>
											</li>
											<li>
												<input type = 'text' name = 'fecha_proxima_t' <?php echo "value = '$proTen' "; ?> size = '30' class = 'ancho date rounded calend'>		
											</li>
											
											<li>
												<label>No. Tarjeta de Circulación: <font color = '#FF0000'>*</font></label>
											</li>
											<li>
												<table border = '0' align = 'left'>
													<tr>
														<td>
															<input type = 'text' name = 'tarjeta' <?php echo "value = '$TarjetaCirculacion' "; ?> size = '30' class = 'rounded'>
														</td>
														<td>  </td>
													</tr>
												</table>
											</li>
											
											<li>
												<label>Tipo de vehículo: <font color = '#FF0000'>*</font></label>
											</li>
											<li>
												<table border = '0' align = 'left'>
													<tr>
														<td>
															<input type = 'text' name = 'tipo_v' <?php echo "value = '$tipo_v' "; ?> size = '30' class = 'rounded' OnKeyUp = 'autotipo(this.value,TipoV)'>
														</td>
														<td id = 'TipoV' width = '22'>  </td>
														<td> <a class = "tooltip" > <font size = '4' color = '#0000FF'>(?)</font> <span class="custom help"> <img src = "../../imagenes/Help.png" alt = "Help" height="35" width="35" />Ingresa el tipo de vehículo:<br>Automóvil ó<br>Camioneta</span></a></td>
													</tr>
												</table>
											</li>
											
											<li>
												<label>Activo:</label> 
																	
													<li class='radio' >
														Si<input type='radio' name='activo' value='Si' checked> 
														No<input type='radio' name='activo' value='No' disabled = 'disabled'>
													</li>
											</li>
									
											<li>									
												<input type = 'submit' value = 'Modificar' class = 'boton rounded1'>
											</li>
											
											<div id="campos">
												* Campos Obligatorios
											</div>
										</ul>
									</form>
								</fieldset>
							</center>

							<div id = 'ajax'></div>
									
								<script type = "text/javascript">
									$(function (modificar) {
										$('#modPv').submit(function (modificar) {
											modificar.preventDefault()
											$('#ajax').load('modificar_plantaVehicular2.php?' + $('#modPv').serialize())
										})
									})
								</script>

				</div>
			</div>

			<div id="pie">
				Sistema de Inventario IAPEM
			</div>
		</div>
	</body>
</html>