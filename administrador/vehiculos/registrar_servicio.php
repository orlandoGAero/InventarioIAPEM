		...
		<div id="contenedor">
			...
			<div id="contenido">
				...
				<div id="derecha11">
					...
					<center>
						<fieldset><legend>Registrar Servicios</legend>
							<form action = '' name = 'altaServ' id = 'altaServ' method = 'POST' target = '_self'>
								<ul>
											...
											$filas = mysql_num_rows($ejecutar);
												...
												if( $filas==0 )
												{
													$IDservicio = 1;
															...
															$IDservicio = mysql_result($ejecutar, 0, 'IDservicio');
															$IDservicio = $IDservicio+1;
												}
										?>

									<li>
										<input type = 'text' name = 'IDservicio' <?php echo "value = '$IDservicio' "?> readonly = 'readonly' size='30' class = 'rounded' />
									</li>
									...
									<li>
										...
										<?php
											...
											$consultaV = "	SELECT codigoNuevoV,placas
															FROM planta_vehicular
															WHERE activo = 'Si' ";
											$ejecutarV = mysql_query($consultaV) or die (mysql_error());
											$filasV = mysql_num_rows($ejecutarV);
											echo "	<select name = 'codigoNuevoV' id = 'codigoNuevoV' class = 'select'>
											...
											echo " <option value = '$codigoNuevoV'> $placas </option> ";
											}
										?>
									...
									<li>
										<label> Fecha de Servicio: <font color='#FF0000'>*</font> </label>
									</li>
									<li>
										<input type = 'text' name = 'fechaS' size = '30' class = 'ancho date rounded calend'>
									</li>

									<li>
										<label> Tipo de Servicio: <font color='#FF0000'>*</font> </label>
									</li>
									<li>
										<textarea name='tipo_servicio' rows='7' cols='55'></textarea>
									</li>
									...
									<li>
										<table border='0' align='left'>
												<tr>
													<td>
														<input type = 'text' name = 'costo' size = '30' class = 'rounded' OnKeyUp = 'cost(this.value,cos)'/>	
													</td>
					...
					<!--crear una division que se llame ajax, debe de ir antes o despues del formulario-->
						<div id="ajax"></div>
							<script type="text/javascript">
								$(function (guardar) {
									$('#altaServ').submit(function (guardar) {
										guardar.preventDefault()
										$('#ajax').load('registrar_servicio2.php?' + $('#altaServ').serialize())
									})
								})
							</script>
							
							<!--Script para mostrar la tabla de vehículo-->
							<script type="text/javascript">
								$(function (e) {
								$('#codigoNuevoV').change(function (e) {
								$('#result').load('tabla_vehiculo.php?codigoNuevoV=' + this.options[this.selectedIndex].value)
									})
								})
							</script>
